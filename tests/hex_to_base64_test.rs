...
#[test]
fn big_hex() {
    let symbols: Vec<String> = "ABCDEF0123456789".as_bytes().iter().map(|c| String::from_utf8(vec![c.clone()]).unwrap()).collect();
    let input = (0..40960).map(|_| symbols.choose(&mut rand::rng()).unwrap()).map(|c| c.clone()).collect::<Vec<_>>().concat();
    println!("Input {}",input);
    let res = htb64::hex_to_base64(input.as_bytes());
